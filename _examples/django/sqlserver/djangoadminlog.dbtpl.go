...
func (dal *DjangoAdminLog) Insert(ctx context.Context, db DB) error {
	...
	rows, err := db.QueryContext(ctx, sqlstr, dal.ActionTime, dal.ObjectID, dal.ObjectRepr, dal.ActionFlag, dal.ChangeMessage, dal.ContentTypeID, dal.UserID)
	if err != nil {
		return logerror(err)
	}
	defer rows.Close()
...
func (dal *DjangoAdminLog) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE django.django_admin_log SET ` +
		`action_time = @p1, object_id = @p2, object_repr = @p3, action_flag = @p4, change_message = @p5, content_type_id = @p6, user_id = @p7 ` +
		`WHERE id = @p8`
	// run
...
func (dal *DjangoAdminLog) Upsert(ctx context.Context, db DB) error {
	...
	// upsert
	const sqlstr = `MERGE django.django_admin_log AS t ` +
		`USING (` +
		`SELECT @p1 id, @p2 action_time, @p3 object_id, @p4 object_repr, @p5 action_flag, @p6 change_message, @p7 content_type_id, @p8 user_id ` +
		`) AS s ` +
		`ON s.id = t.id ` +
		`WHEN MATCHED THEN ` +
		`UPDATE SET ` +
...
//
// Generated from foreign key 'django_admin_log_content_type_id_c4bce8eb_fk_django_content_type_id'.
