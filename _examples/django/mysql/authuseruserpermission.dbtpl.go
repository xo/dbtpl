...
func (auup *AuthUserUserPermission) Insert(ctx context.Context, db DB) error {
	...
	res, err := db.ExecContext(ctx, sqlstr, auup.UserID, auup.PermissionID)
	if err != nil {
		return logerror(err)
	}
	// retrieve id
...
func (auup *AuthUserUserPermission) Upsert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO django.auth_user_user_permissions (` +
		...
		`user_id = VALUES(user_id), permission_id = VALUES(permission_id)`
	// run
...
//
// Generated from index 'auth_user_user_permi_permission_id_1fbb5f2c_fk_auth_perm'.
