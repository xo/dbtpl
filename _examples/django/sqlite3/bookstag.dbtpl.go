...
func (bt *BooksTag) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO books_tags (` +
		`id, book_id, tag_id` +
		`) VALUES (` +
		`$1, $2, $3` +
		`)`
	...
	res, err := db.ExecContext(ctx, sqlstr, bt.ID, bt.BookID, bt.TagID)
	if err != nil {
		return logerror(err)
	}
	// retrieve id
...
func (bt *BooksTag) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE books_tags SET ` +
		`book_id = $1, tag_id = $2 ` +
		`WHERE id = $3`
	// run
...
func (bt *BooksTag) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM books_tags ` +
		`WHERE id = $1`
	// run
	logf(sqlstr, bt.ID)
...
func (bt *BooksTag) Tag(ctx context.Context, db DB) (*Tag, error) {
	return TagByTagID(ctx, db, int(bt.TagID))
}
