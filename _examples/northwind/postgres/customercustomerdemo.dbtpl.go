...
// CustomerCustomerDemo represents a row from 'public.customer_customer_demo'.
...
func (ccd *CustomerCustomerDemo) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM public.customer_customer_demo ` +
...
// CustomerCustomerDemoByCustomerIDCustomerTypeID retrieves a row from 'public.customer_customer_demo' as a [CustomerCustomerDemo].
//
...
func CustomerCustomerDemoByCustomerIDCustomerTypeID(ctx context.Context, db DB, customerID, customerTypeID string) (*CustomerCustomerDemo, error) {
	...
	const sqlstr = `SELECT ` +
		`customer_id, customer_type_id ` +
		`FROM public.customer_customer_demo ` +
		`WHERE customer_id = $1 AND customer_type_id = $2`
	// run
