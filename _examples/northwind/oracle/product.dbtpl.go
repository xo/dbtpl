...
func (p *Product) Upsert(ctx context.Context, db DB) error {
	...
	// upsert
	const sqlstr = `MERGE northwind.productst ` +
		`USING (` +
		`SELECT :1 product_id, :2 product_name, :3 supplier_id, :4 category_id, :5 quantity_per_unit, :6 unit_price, :7 units_in_stock, :8 units_on_order, :9 reorder_level, :10 discontinued ` +
		`FROM DUAL ) s ` +
