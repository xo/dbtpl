...
func (t *Territory) Upsert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO northwind.territories (` +
		...
		`territory_id = VALUES(territory_id), territory_description = VALUES(territory_description), region_id = VALUES(region_id)`
	// run
	logf(sqlstr, t.TerritoryID, t.TerritoryDescription, t.RegionID)
	if _, err := db.ExecContext(ctx, sqlstr, t.TerritoryID, t.TerritoryDescription, t.RegionID); err != nil {
		return logerror(err)
...
func TerritoriesByRegionID(ctx context.Context, db DB, regionID int16) ([]*Territory, error) {
	// query
	const sqlstr = `SELECT ` +
		`territory_id, territory_description, region_id ` +
	...
	for rows.Next() {
		...
		// scan
		if err := rows.Scan(&t.TerritoryID, &t.TerritoryDescription, &t.RegionID); err != nil {
			return nil, logerror(err)
		}
		res = append(res, &t)
	}
